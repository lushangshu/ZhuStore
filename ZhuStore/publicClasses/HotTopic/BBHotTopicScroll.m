...
@interface BBHotTopicScroll ()<UIScrollViewDelegate>
{
    float P_x; //标记位置
    float P_y;
}
...
@implementation BBHotTopicScroll
...
- (void)nextLabel {
    ...
    if(count == _titleArray.count){
        count = 0;
        P_x = 0;
        P_y = 0;
        [self.ccpScrollView setContentOffset:CGPointMake(P_x, P_y) animated:NO];
        ...
        P_y+= self.ccpScrollView.frame.size.height;
        //NSLog(@"oldPoint is %f,%f",oldPoint.x,oldPoint.y );
        [self.ccpScrollView setContentOffset:CGPointMake(P_x, P_y) animated:YES];
        count = count +1;
    }
